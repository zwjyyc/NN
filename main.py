...
class Model:
	...
	softmax_inputs = [ ]
		...
		if args.text_type == "sent":
                    softmax_inputs.append(prev_output[-1])
		elif args.text_type == "doc":
...
def main(args):
    ...
    assert args.text_type, "Text type required"
    ...
    if args.train:
	if args.text_type == "sent":
            train_x, train_y = load_sent_corpus(embedding_layer, args.train)
	elif args.text_type == "doc":
	    train_x, train_y = load_doc_corpus(embedding_layer, args.train)

    if args.dev:
	if args.text_type == "sent":
            dev_x, dev_y = load_sent_corpus(embedding_layer, args.dev)
	elif args.text_type == "doc":
	    dev_x, dev_y = load_doc_corpus(embedding_layer, args.dev)

    if args.test:
	if args.text_type == "sent":
            test_x, test_y = load_sent_corpus(embedding_layer, args.test)
	elif args.text_type == "doc":
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--text_type",
	    type = str,
	    default = "doc",
	    help = "text type (sent, doc)"
	)
